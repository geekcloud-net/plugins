...
class YITH_COG_Report_Data_Product extends YITH_COG_Admin_Report {
    ...
    public function get_chart_widgets() {
        ...
        if ( ! empty( $this->product_ids ) ) {
            $widgets[] = array(
                'title'    => __( 'Showing reports for:', 'yith-cost-of-goods-for-woocommerce' ),
                'callback' => array( $this, 'current_filters' ),
            );
        }

        $widgets[] = array(
            'title'    => __( 'Product Search', 'yith-cost-of-goods-for-woocommerce' ),
    ...
    public function current_filters() {
        ...
        echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'product_ids' ) ) . '">' . __( 'Reset', 'yith-cost-of-goods-for-woocommerce' ) . '</a></p>';
    }
    ...
    public function output_current_filters_widget() {
        ...
        printf( '<p><strong>%1$s</strong></p><p><a class="button" href="%2$s">%3$s</a></p>', implode( ', ', $product_titles ), esc_url( remove_query_arg( 'product_ids' ) ), __( 'Reset', 'yith-cost-of-goods-for-woocommerce' ) );
    }
...
//            'order_by' => 'post_date ASC',
        ...
        foreach ( $item_qty_array as $key => $value ){
            if ($item_qty_array[$key] < 0 ){
                ...
                $item_cost_array[$key] = -(1) * $cost_value;
                ...
                $cost_value =  $item_cost_array[$key];
                $price_value =  $item_price_array[$key];
                $item_cost_array[$key] = $qty_value * $cost_value;
                $item_price_array[$key] = $qty_value * $price_value;
            }
        }
