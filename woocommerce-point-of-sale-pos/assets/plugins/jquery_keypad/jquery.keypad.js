...
(function ($) { // hide the namespace
	...
	return ch.toUpperCase();
        ...
        _optionsChanged: function (elem, inst, options) {
            ...
            elem.off('.' + inst.name).siblings('.' + this._appendClass).remove().end().siblings('.' + this._triggerClass).remove();
            ...
            if (!inst._inline) {
                if (inst.options.showOn == 'focus' || inst.options.showOn == 'both') {
                    ...
                    elem.on('focus.' + inst.name, this.show).on('keydown.' + inst.name, this._doKeyDown);
                }
            ...
            elem[inst.options.keypadOnly ? 'attr' : 'removeAttr']('readonly', true).on('setData.' + inst.name, function (event, key, value) {
        ...
        _preDestroy: function (elem, inst) {
            ...
            elem.siblings('.' + this._appendClass).remove().end().siblings('.' + this._triggerClass).remove().end().prev('.' + this._inlineEntryClass).remove();
        ...
        _updateKeypad: function (inst) {
            ...
            inst._mainDiv.empty().append(this._generateHTML(inst)).removeClass().addClass(inst.options.keypadClass +
        ...
        hide: function (elem, duration) {
            ...
            this._curInst = null;
            $('.keypad-popup').attr('style', 'display: none;');
        },
        ...
        _generateHTML: function (inst) {
            ...
            for (var i = 0; i < layout.length; i++) {
                ...
                for (var j = 0; j < keys.length; j++) {
                    ...
                    if (keyDef) {
                        html += (keyDef.action ? '<button type="button" class="button ' + this._specialClass +
                        ...
                        html += '<button type="button" class="button ' + this._keyClass +
                            (inst.options.useThemeRoller ? ' ui-corner-all ui-state-default' : '') + '">' +
    ...
    $(function () {
        $(document.body).append(plugin.mainDiv).mousedown(plugin._checkExternalClick);
    });
