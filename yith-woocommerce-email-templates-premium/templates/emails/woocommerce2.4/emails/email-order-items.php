...
foreach ( $items as $item_id => $item ) :
	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		<tr>
			...
			?></td>
			<td class="yith-wcet-order-items-table-element-quantity" style="vertical-align:middle;"><?php echo $item['qty'] ;?></td>
			<td class="yith-wcet-order-items-table-element-price" style="vertical-align:middle;"><?php echo $order->get_formatted_line_subtotal( $item ); ?></td>
		</tr>
		<?php
	}
	...
	if ( $show_purchase_note && is_object( $_product ) && ( $purchase_note = get_post_meta( $_product->id, '_purchase_note', true ) ) ) : ?>
		<tr>
			<td class="yith-wcet-order-items-table-element" colspan="3" style="text-align:left; vertical-align:middle;"><?php echo wpautop( do_shortcode( wp_kses_post( $purchase_note ) ) ); ?></td>
		</tr>
	<?php endif; ?>
