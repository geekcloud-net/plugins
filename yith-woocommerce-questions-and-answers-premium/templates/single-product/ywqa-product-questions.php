	...
	<div class="questions-section">
		...
		<div id="ywqa_question_list">
			<?php if ( $question_count ) : ?>
				...
				<?php if ( 0 == $item_shown ) : ?>
					<p class="no-questions"><?php _e ( "No question has still received an answer.", 'yith-woocommerce-questions-and-answers' ); ?></p>
				<?php endif; ?>
				...
				<?php if ( $item_shown < $question_count ) : ?>
					<div id="show-all-questions">
						<a class="show-questions"
						   href="<?php echo esc_url ( add_query_arg ( "show-all-questions", $product_id, get_permalink ( $product_id ) ) ); ?>"
						   title="<?php echo sprintf ( __ ( "Show all %d questions", 'yith-woocommerce-questions-and-answers' ), $question_count ); ?>"><?php echo sprintf ( __ ( "Show all %d questions %s", 'yith-woocommerce-questions-and-answers' ), $question_count, $unanswered_count ? sprintf ( __ ( '(%d without an answer)', 'yith-woocommerce-questions-and-answers' ), $unanswered_count ) : '' ); ?></a>
					</div>
				<?php endif; ?>
			<?php elseif ( ! YITH_YWQA ()->faq_mode ) : ?>
				<p class="woocommerce-noreviews"><?php _e ( 'There are no questions yet, be the first to ask something for this product.', 'yith-woocommerce-questions-and-answers' ); ?></p>
			<?php endif; ?>
		...
		if ( ! YITH_YWQA ()->faq_mode )  : ?>
			<?php if ( get_current_user_id () || YITH_YWQA ()->allow_guest_users ): ?>
				<div id="ask_question">
					<form id="ask_question_form" method="POST">
						...
						<?php wp_nonce_field ( 'ask_question_' . $product_id, 'ask_question' ); ?>
						<div class="ywqa-ask-question">
							
							<p class="ywqa_ask_question_text">
								<label
									for="ywqa_user_content"><?php _e ( 'Your question', 'yith-woocommerce-questions-and-answers' ); ?>
									<span class="required">*</span></label>
								<textarea id="ywqa_user_content" name="ywqa_user_content"
								          placeholder="<?php _e ( 'Do you have any questions? Ask now!', 'yith-woocommerce-questions-and-answers' ); ?>"
							...
							<div class="notify-answers">
								<?php if ( YITH_YWQA ()->enable_user_notification ) : ?>
									<input id="ywqa-notify-user" name="ywqa-notify-user" type="checkbox"
									       ...
									       checked="checked"><?php _e ( "Send me a notification for each new answer.", 'yith-woocommerce-questions-and-answers' ); ?>
								<?php endif; ?>
			...
			<?php else: ?>
				<div class="ywqa-guest-user">
					<?php _e ( "Only registered users are eligible to enter questions", 'yith-woocommerce-questions-and-answers' ); ?>
				</div>
			<?php endif; ?>
		<?php endif; ?>
