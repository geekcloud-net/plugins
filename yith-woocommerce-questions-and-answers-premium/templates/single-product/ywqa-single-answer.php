...
<li id="li-answer-<?php echo $answer->ID; ?>" class="answer-container <?php echo $classes; ?>">
	...
	<div class="answer-content">
		<span class="answer">
			...
			<?php if ( ( YITH_YWQA()->answer_excerpt_length > 0 ) && ( strlen( $answer->content ) > YITH_YWQA()->answer_excerpt_length ) ) :
				echo substr( $content, 0, YITH_YWQA()->answer_excerpt_length ) . '... <a href="#" data-discussion-id="' . $answer->ID . '" class="read-more">' . __( "Read more", 'yith-woocommerce-questions-and-answers' ) . '</a>';
			else:
				echo $content;
			endif;
			?>
	...
	<div class="answer-owner">
		...
		if ( YITH_YWQA()->anonymise_user ) {
			echo sprintf( __( "Answered on %s", 'yith-woocommerce-questions-and-answers' ), $answer_date_html);
		} else {
			echo sprintf( __( "%s answered on %s", 'yith-woocommerce-questions-and-answers' ),
		...
		<div class="clearfix vote-answer">
			<?php if ( $vote_answer ) : ?>
				<div class="answer-helpful">
					...
					<div class="is-answer-helpful">
						<a href="#" class="answer-helpful"
						   data-discussion-id="<?php echo $answer->ID; ?>"
						   data-discussion-vote="1"><?php _e( "YES", 'yith-woocommerce-questions-and-answers' ); ?></a>
						<a href="#" class="answer-not-helpful" data-discussion-id="<?php echo $answer->ID; ?>"
						   data-discussion-vote="-1"><?php _e( "NO", 'yith-woocommerce-questions-and-answers' ); ?></a>
					</div>
				</div>
			<?php endif; ?>
			...
			<?php if ( $report_abuse ) : ?>
				<div class="answer-abuse">
					<a title="<?php _e( "Report an inappropriate content", 'yith-woocommerce-questions-and-answers' ); ?>" href="#" data-discussion-id="<?php echo $answer->ID; ?>"
					   class="report-answer-abuse"><?php _e( "Report", 'yith-woocommerce-questions-and-answers' ); ?></a>
				</div>
			<?php endif; ?>
