	...
	<div id="new-answer-header">
		<div id="parent-question">
			...
			<span class="question-text"><span
					class="question-symbol"><?php _e( "Q", 'yith-woocommerce-questions-and-answers' ); ?></span>
				<?php echo nl2br( $question->content ); ?></span>
				<a class="back-to-product"
				   href="<?php echo esc_url( add_query_arg( "qa", 1, remove_query_arg( array(
					   "show-all-questions",
					   "reply-to-question",
				   ), get_permalink( $question->product_id ) ) ) ); ?>"
				   title="<?php _e( "Back to questions", 'yith-woocommerce-questions-and-answers' ); ?>"><?php _e( "<<< Back to questions", 'yith-woocommerce-questions-and-answers' ); ?>
				</a>
			</div>
			
			<?php if ( YITH_YWQA()->anonymise_user ) : ?>
				<div class="question-owner">
					<?php echo sprintf( __( "Asked on %s", 'yith-woocommerce-questions-and-answers' ),
						'<span class="question-date">' . date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $question->date ) ) . '</span>' ); ?>
				</div>
			<?php else: ?>
				<div class="question-owner">
					<?php echo sprintf( __( "Asked by %s on %s", 'yith-woocommerce-questions-and-answers' ),
		...
		<?php if ( ! $answers_count ) : ?>
			<p class="no-answers"><?php echo apply_filters( 'yith_ywqa_no_answer_text',__( 'There are no answers for this question, be the first to respond.', 'yith-woocommerce-questions-and-answers' )); ?></p>
		<?php endif; ?>
		...
		if ( ! YITH_YWQA()->faq_mode ) : ?>
					...
					<form id="submit_answer_form" method="POST">
						...
						<div class="ywqa-send-answer">
							<p class="ywqa_send_answer_text">
								<label
									for="ywqa_user_content"><?php _e( 'Your answer', 'yith-woocommerce-questions-and-answers' ); ?>
									
									<span class="required">*</span>
								
								</label>
								<textarea
									placeholder="<?php _e( "Type your answer here", 'yith-woocommerce-questions-and-answers' ); ?>"
							...
							<?php if ( ! get_current_user_id() ): ?>
								<p class="ywqa-guest-name-section">
									<label
										for="ywqa-guest-name"><?php _e( 'Name', 'yith-woocommerce-questions-and-answers' ); ?>
									...
									<input id="ywqa-guest-name" name="ywqa-guest-name" class="ywqa-guest-name required"
									       type="text"
									       placeholder="<?php _e( "Enter your name", 'yith-woocommerce-questions-and-answers' ); ?>"
									       value="" aria-required="true" <?php echo YITH_YWQA()->mandatory_guest_data ? "required" : ""; ?>>
								</p>
								<p class="ywqa-guest-email-section">
									<label
										for="ywqa-guest-email"><?php _e( 'Email', 'yith-woocommerce-questions-and-answers' ); ?>
										<?php if ( YITH_YWQA()->mandatory_guest_data ): ?>
											<span class="required">*</span>
										<?php endif; ?>
									</label>
									<input id="ywqa-guest-email" name="ywqa-guest-email"
									       ...
									       type="text"
									       placeholder="<?php _e( "Enter your email", 'yith-woocommerce-questions-and-answers' ); ?>"
									       value="" aria-required="true" <?php echo YITH_YWQA()->mandatory_guest_data ? "required" : ""; ?>>
								</p>
							<?php endif; ?>
			...
			<?php else: ?>
				<div class="ywqa-guest-user">
					<?php echo apply_filters( 'yith_wcqa_unable_submit_answer_message',__( "Only registered users are eligible to enter answers", 'yith-woocommerce-questions-and-answers' )); ?>
				</div>
			<?php endif; ?>
		<?php endif; ?>
	</div>
	...
	<div id="ywqa_answer_list">
		...
		<span
			class="answer-list-count"><?php echo sprintf( _n( "%s answer shown", "%s answers shown", $answers_count, 'yith-woocommerce-questions-and-answers' ), $answers_count ); ?></span>
				...
				<div class="order-by">
					<span><?php _e( "Sort by:", 'yith-woocommerce-questions-and-answers' ); ?></span>
					<a class="ywqa-question-order" data-order="useful"><?php _e( "Most useful", 'yith-woocommerce-questions-and-answers' ); ?></a>
					|
					<a class="ywqa-question-order" data-order="recent"><?php _e( "Most recent", 'yith-woocommerce-questions-and-answers' ); ?></a>
					|
					<a class="ywqa-question-order" data-order="oldest"><?php _e( "Oldest", 'yith-woocommerce-questions-and-answers' ); ?></a>
				</div>
			</div>
