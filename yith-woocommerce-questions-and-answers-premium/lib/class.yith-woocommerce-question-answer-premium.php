...
if ( ! class_exists( 'YITH_WooCommerce_Question_Answer_Premium' ) ) {
	...
	class YITH_WooCommerce_Question_Answer_Premium extends YITH_WooCommerce_Question_Answer {
		...
		/**
		 * How much questions and answer to show after a text filter
		 *
		 * @var bool
		 */
		...
		protected function __construct() {
			...
			add_action( 'yith_questions_answers_custom_column_content', array(
				$this,
				'show_custom_table_columns',
			), 10, 2 );
			...
			//  Add ajax callback handler for back-end operation
			add_action( 'wp_ajax_filter_content', array(
				$this,
				'filter_content_callback'
			) );
			add_action( 'wp_ajax_nopriv_filter_content', array(
				$this,
				'filter_content_callback',
			) );
		...
		public function product_dropdown() {
			...
			$reset_button = esc_url( add_query_arg( array( 'post_type' => $_REQUEST['post_type'] ), admin_url( 'edit.php' ) ) );
			?>
			...
			$args = array(
				...
				'data-placeholder' => __( 'Search for a product&hellip;', 'yith-woocommerce-questions-and-answers' ),
			...
			<!--						   style="margin: 1px 8px 0 0;">--><?php //_e ( 'Reset', 'yith-woocommerce-questions-and-answers' ); ?><!--</a>-->
		...
		public function add_custom_views( $views ) {
			...
			if ( $count ) {
				$views["unanswered"] = sprintf( "<a href='edit.php?post_type=question_answer&filter_question=unanswered_q'>%s<span class='count'> (%s)</span></a>",
					__( "Unanswered questions", 'yith-woocommerce-questions-and-answers' ), $count );
			}
			...
			if ( $count ) {
				$views["questions"] = sprintf( "<a href='edit.php?post_type=question_answer&filter_question=questions'>%s<span class='count'> (%s)</span></a>",
					__( "Questions", 'yith-woocommerce-questions-and-answers' ), $count );
			}
			...
			if ( $count ) {
				$views["answers"] = sprintf( "<a href='edit.php?post_type=question_answer&filter_question=answers'>%s<span class='count'> (%s)</span></a>",
					__( "Answers", 'yith-woocommerce-questions-and-answers' ), $count );
			}
		...
		public function add_rows_actions( $actions, $post ) {
			...
			if ( 'inappropriate' === $post->post_status ) {
				$actions["restore-appropriate"] = "<a title='" . esc_attr__( 'Restore as appropriate content', 'yith-woocommerce-questions-and-answers' ) . "' href='" . wp_nonce_url( admin_url( "admin.php?action=set-appropriate-content&post_type=" . YWQA_CUSTOM_POST_TYPE_NAME . "&id=" . $post->ID ), 'set-appropriate-status-' . $post->ID ) . "'>" . __( 'Set appropriate', 'yith-woocommerce-questions-and-answers' ) . "</a>";
			} elseif ( 'publish' === $post->post_status ) {
				$actions["set-inappropriate"] = "<a title='" . esc_attr__( 'Set as inappropriate content', 'yith-woocommerce-questions-and-answers' ) . "' href='" . wp_nonce_url( admin_url( "admin.php?action=set-inappropriate-content&post_type=" . YWQA_CUSTOM_POST_TYPE_NAME . "&id=" . $post->ID ), 'set-appropriate-status-' . $post->ID ) . "'>" . __( 'Set inappropriate', 'yith-woocommerce-questions-and-answers' ) . "</a>";
			}
			
			if ( 'unapproved' === $post->post_status ) {
				$actions["restore-approved"] = "<a title='" . esc_attr__( 'Restore as approved content', 'yith-woocommerce-questions-and-answers' ) . "' href='" . wp_nonce_url( admin_url( "admin.php?action=set-approved-content&post_type=" . YWQA_CUSTOM_POST_TYPE_NAME . "&id=" . $post->ID ), 'set-approved-status-' . $post->ID ) . "'>" . __( 'Approve', 'yith-woocommerce-questions-and-answers' ) . "</a>";
			} elseif ( 'publish' === $post->post_status ) {
				$actions["set-unapproved"] = "<a title='" . esc_attr__( 'Set as unapproved content', 'yith-woocommerce-questions-and-answers' ) . "' href='" . wp_nonce_url( admin_url( "admin.php?action=set-unapproved-content&post_type=" . YWQA_CUSTOM_POST_TYPE_NAME . "&id=" . $post->ID ), 'set-approved-status-' . $post->ID ) . "'>" . __( 'Unapprove', 'yith-woocommerce-questions-and-answers' ) . "</a>";
			}
		...
		public function register_custom_post_statuses() {
			...
			register_post_status( 'inappropriate', array(
				'label'                     => _x( 'Inappropriate', 'yith-woocommerce-questions-and-answers' ),
				'public'                    => true,
			...
			register_post_status( 'unapproved', array(
				'label'                     => _x( 'Unapproved', 'yith-woocommerce-questions-and-answers' ),
				'public'                    => true,
				'exclude_from_search'       => false,
				'show_in_admin_all_list'    => true,
				'show_in_admin_status_list' => true,
		...
		public function unsubscribe_customer_for_product_feedback() {
			...
			if ( ! wp_verify_nonce( $_POST["_wpnonce"], $nonce_form ) ) {
				wp_die( "<p>" . __( "An error occurred. The requested resource does not exist or the content is deprecated.", 'yith-woocommerce-questions-and-answers' ) . "</p>" );
			}
			...
			if ( strcasecmp( $form_email, $email ) == 0 ) {
				...
				wp_die( "<p>" . __( "The email address is not valid.", 'yith-woocommerce-questions-and-answers' ) . "</p>" );
			}
		}
		...
		public function unsubscribe_shortcode( $atts ) {
			...
			if ( isset( $_GET['unsubscribe'] ) ) {
				?>
				<p>
					<?php _e( 'Unsubscription completed. You will not receive any more answering requests for the selected products.', 'yith-woocommerce-questions-and-answers' ); ?>
				</p>
				<?php
				return;
			}
			...
			<p>
				<?php _e( 'Please, confirm your email address to stop receiving answering requests for the questions about products you have bought:', 'yith-woocommerce-questions-and-answers' ); ?>
			</p>
			<form method="post" action="">
				<p class="form-row form-row-wide">
					<label
						for="unsubscribe_email"><?php _e( 'Email address', 'yith-woocommerce-questions-and-answers' ); ?>
				...
				<p>
					...
					<input type="submit" class="button" name="unsubscribe"
					       value="<?php _e( 'Unsubscribe', 'yith-woocommerce-questions-and-answers' ); ?>" />
				</p>
			</form>
			<?php
		}
		...
		public function create_custom_page() {
			...
			wc_create_page( esc_sql( _x( 'questions_and_answers_unsubscription', 'Page slug', 'yith-woocommerce-questions-and-answers' ) ),
				'ywqa_unsubscribe_page_id',
				_x( 'Questions & Answers unsubscription', 'Page title', 'yith-woocommerce-questions-and-answers' ),
				'[ywqa_unsubscribe]',
		...
		public function init_plugin_settings() {
			...
			$this->enable_search        = ( "yes" === get_option( "ywqa_enable_search", "no" ) );
			$this->search_items_to_show = get_option( "ywqa_search_items", 1 );
		...
		public function show_custom_table_columns( $column_name, $post_ID ) {
			...
			switch ( $column_name ) {
				...
				case 'actions' :
					...
					if ( ! $product ) {
						_e( "Product not found!", 'yith-woocommerce-questions-and-answers' );
					...
					echo sprintf( '<span class="for-product">%s</span><a class="view-product" target="_blank" href="%s" title="%s">%s</a>',
						__( "Product: ", 'yith-woocommerce-questions-and-answers' ),
						$product_edit_link,
						$product_title,
						$product_title );
					...
					if ( $discussion instanceof YWQA_Answer ) {
						...
						echo " <br>" . sprintf( '<span class="response-to">%s</span><a class="response-to" href="%s" title="%s">%s</a>',
								__( "Response to: ", 'yith-woocommerce-questions-and-answers' ),
								$question_edit_link,
								$question_title,
								$question_title );
					}
		...
		public function add_custom_table_columns( $columns ) {
			...
			$columns["qa_author"] = __( "Author", 'yith-woocommerce-questions-and-answers' );
			$columns["upvotes"]   = __( "Upvotes", 'yith-woocommerce-questions-and-answers' );
			$columns["downvotes"] = __( "Downvotes", 'yith-woocommerce-questions-and-answers' );
			$columns["abuse"]     = __( "Abuse reports", 'yith-woocommerce-questions-and-answers' );
		...
		public function enqueue_styles_scripts() {
			...
			$localize_params = array(
				...
				'abuse_response'       => __( "Thanks", 'yith-woocommerce-questions-and-answers' ),
				'discussion_complete'  => __( "Your content has been sent correctly.", 'yith-woocommerce-questions-and-answers' ),
				'discussion_error'     => __( "An error has occurred, your content has not been added correctly.", 'yith-woocommerce-questions-and-answers' ),
				'content_is_empty'     => __( "Please write your text before submitting it.", 'yith-woocommerce-questions-and-answers' ),
				'guest_name_message'   => __( "Please enter your name", 'yith-woocommerce-questions-and-answers' ),
				'guest_email_message'  => __( "Please enter your e-mail", 'yith-woocommerce-questions-and-answers' ),
				'mandatory_guest_data' => $this->mandatory_guest_data,
				'recaptcha_not_valid'  => __( "Please confirm you are human solving the reCaptcha.", 'yith-woocommerce-questions-and-answers' ),
		...
		public function get_helpful_answer_text( $discussion ) {
			...
			if ( $total > 0 ) {
				return sprintf( __( "%d on %d found it useful. What do you think?", 'yith-woocommerce-questions-and-answers' ), $stats["yes"], $total );
			}
			
			return __( "Do you think this answer is useful?", 'yith-woocommerce-questions-and-answers' );
		}
			...
			if ( "yes" === get_option( "ywqa_enable_question_vote", "no" ) ) :
				...
				<div class="question-votes">
					<a class="vote-question vote-yes"
					   title=" <?php _e( "Upvote this question", 'yith-woocommerce-questions-and-answers' ); ?>"
					   href="#"
					...
					<a class="vote-question vote-no"
					   title="<?php _e( "Downvote this question", 'yith-woocommerce-questions-and-answers' ); ?>"
					   href="#" data-discussion-id="<?php echo $question->ID; ?>" data-discussion-vote="-1"></a>
				</div>
				<?php
			endif;
		...
		public function filter_content_callback() {
			if ( ! YITH_YWQA()->enable_search ) {
				return;
			}
			...
			$filter_text = trim( sanitize_text_field( $_POST['text'] ) );
			
			if ( empty( $filter_text ) ) {
				$result['code'] = - 1;
			} else {
				$query_filter = '';
				$strings      = explode( " ", $filter_text );
				foreach ( $strings as $string ) {
					if ( ! empty( $query_filter ) ) {
						$query_filter .= ' or ';
					}
					$query_filter .= sprintf( "(post_content like '%%%s%%')", $string );
				}
			...
			$query_limit   = '';
			$visible_items = YITH_YWQA()->search_items_to_show;
			if ( $visible_items ) {
				$query_limit = sprintf( " limit %d,%d ", 0, $visible_items );
			}
			
			$order_by_query = " order by post_date DESC ";
			
			global $wpdb;
			$query = $wpdb->prepare( "select ID, post_parent as parent_id, meta_value as product_id 
				from {$wpdb->posts} p left join {$wpdb->postmeta} pm on pm.post_id = p.ID
				where post_status = 'publish' and
				      post_type = %s and
				      pm.meta_key = %s and
				      pm.meta_value = %d and " . $query_filter . $order_by_query . $query_limit,
				YWQA_CUSTOM_POST_TYPE_NAME,
				YWQA_METAKEY_PRODUCT_ID,
				$product_id
			);
			...
			foreach ( $items as $item ) {
				if ( $product_id == $item['post_parent'] ) {
					$results[] = new YWQA_Question( $item["ID"] );
				} else {
					$results[] = new YWQA_Answer( $item["ID"] );
				}
			}
			
			ob_start();
			wc_get_template( 'single-product/ywqa-search-results.php',
				array(
					'items' => $results,
				),
				'', YITH_YWQA_TEMPLATES_DIR );
		...
		function get_answers_callback() {
			...
			ob_start();
			$answers_count = $question->get_answers_to_show_count();
		...
		function verify_recaptcha() {
			...
			if ( $this->recaptcha_enabled ) {
				...
				if ( ! isset( $_POST["recaptcha"] ) ) {
					...
					wp_send_json( array(
						"code"    => - 1,
						"message" => __( "An error occurred during the reCaptcha validation. Recaptcha token not set.", "ywau" ),
					) );
				}
				...
				if ( ! $resp->isSuccess() ) {
					...
					wp_send_json( array(
						"code"    => - 1,
						"message" => __( "An error occurred during the reCaptcha validation. The token is not valid.", "ywau" ),
					) );
				}
			}
		...
		public function submit_question_callback() {
			...
			if ( ! $current_user_id ) {
				$args['discussion_author_name']  = $this->allow_guest_users && isset( $_POST['name'] ) ? $_POST['name'] : __( "Anonymous user", 'yith-woocommerce-questions-and-answers' );
			...
			wp_send_json( array(
				...
				"message"          => __( "Thanks for your post. It has been sent to site administrator for approval.", 'yith-woocommerce-questions-and-answers' ),
			) );
		}
		...
		function update_discussion_attribute_callback() {
			...
			if ( ! is_user_logged_in() ) {
				...
				$goto_url = wp_login_url( $return_path );
			...
			switch ( $action ) {
				...
				case "report_answer_abuse":
					...
					wp_send_json( array(
						"code"  => 1,
						"value" => __( "Thanks!", 'yith-woocommerce-questions-and-answers' )
					) );
