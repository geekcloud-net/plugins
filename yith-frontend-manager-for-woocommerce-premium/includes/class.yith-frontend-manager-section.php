...
if ( ! class_exists( 'YITH_Frontend_Manager_Section' ) ) {
	...
	class YITH_Frontend_Manager_Section {
		...
		public function print_shortcode( $atts = array(), $content = '', $tag ) {
			trigger_error( sprintf( 'The %s() method must be overridden in child class', __FUNCTION__ ), E_USER_WARNING );
		}
		...
		public function is_current( $subsection = '' ) {
			global $wp;
			$is_current = isset( $wp->query_vars[ $this->slug ] );

			if ( $subsection && $is_current && isset( $this->_subsections[ $subsection ]['slug'] ) ) {
