<div id="email-actions-box" class="submitbox">
			...
			?>
				...
				<div id="post-status-select" class="hide-if-js">
					...
					<select name='post_status' id='post_status'>
						<option<?php selected( in_array( $email->status, array( 'draft', 'auto-draft', FUE_Email::STATUS_ACTIVE ) ) ); ?> value='<?php esc_attr_e(FUE_Email::STATUS_ACTIVE); ?>'><?php _e('Active', 'follow_up_emails') ?></option>
	...
	<div id="publishing-actions">
		...
		<div id="publishing-action">
			<span class="spinner"></span>
			<input type="submit" class="button save_email button-primary" name="save" value="<?php _e( 'Save Email', 'follow-up-emails' ); ?>" <?php if ( empty( $email->type ) ) echo 'disabled' ?> />
		</div>
		<div class="clear"></div>
	</div>
</div>
