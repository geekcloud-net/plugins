...
class FUE_API_Authentication {
	 ...
	 * @throws Exception
	 */
	public static function get_user_by_consumer_key( $consumer_key ) {
		global $wpdb;
	 ...
	 * Matches against FUE first, then checks against WC.
	 *
	 * @since 4.1
	 * @param WP_User $user
	 ...
	 * @return bool
	 */
	public static function is_consumer_secret_valid( WP_User $user, $consumer_secret ) {
		global $wpdb;

		$valid = false;

		if ( get_user_meta( $user->ID, 'fue_api_consumer_secret', true ) === $consumer_secret ) {
			$valid = true;
		} else {
			$sql = $wpdb->prepare( "SELECT COUNT(*)
					FROM {$wpdb->prefix}woocommerce_api_keys c
					WHERE consumer_secret = %s AND user_id = %d", $consumer_secret, $user->ID );
