...
class FUE_Coupons {
	...
	public static function add_prefix( $code, $coupon, $email_data ) {
		if ( '' != $coupon->coupon_prefix ) {
			...
			if ( isset( $email_data['order_id'] ) ) {
				$prefix = str_replace( '{order_number}', apply_filters( 'woocommerce_order_number', $email_data['order_id'] ), $prefix );
			}
	...
	public function settings_tab_controller( $tab ) {
		...
		if ( 'coupons' === $tab ) {
			...
			fue_settings_footer();
		} elseif ( 'new-coupon' === $tab ) {
			fue_settings_header( $tab );
			FUE_Coupons::coupon_form_html();
			fue_settings_footer();
		} elseif ( 'edit-coupon' === $tab ) {
			fue_settings_header( $tab );
			FUE_Coupons::coupon_form_html();
			fue_settings_footer();
		}
