	...
	class YWMMQ_Tags_Bulk_Ops {
		...
		public function output() {
				...
				'custom_columns'   => array(
					...
					'column_override_qty' => function ( $item, $me ) {
						...
						if ( $item['override_qty'] == 'yes' ) {
							$class  = 'show';
							$tip    = __( 'Yes', 'yith-woocommerce-minimum-maximum-quantity' );
							$min    = YITH_WMMQ()->get_term_meta( $item['ID'], '_ywmmq_tag_minimum_quantity', true );
							$max    = YITH_WMMQ()->get_term_meta( $item['ID'], '_ywmmq_tag_maximum_quantity', true );
							$step   = YITH_WMMQ()->get_term_meta( $item['ID'], '_ywmmq_tag_step_quantity', true );
					...
					'column_override_val' => function ( $item, $me ) {
						...
						if ( $item['override_val'] == 'yes' ) {
							$class  = 'show';
							$tip    = __( 'Yes', 'yith-woocommerce-minimum-maximum-quantity' );
							$min    = YITH_WMMQ()->get_term_meta( $item['ID'], '_ywmmq_tag_minimum_value', true );
							$max    = YITH_WMMQ()->get_term_meta( $item['ID'], '_ywmmq_tag_maximum_value', true );
			...
			if ( ! empty( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], basename( __FILE__ ) ) ) {
				...
				if ( $item_valid !== true ) {
					...
					foreach ( $tag_ids as $tag_id ) {
						...
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_exclusion', $exclusion );
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_quantity_limit_override', $override_qty );
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_minimum_quantity', $minimum_qty );
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_maximum_quantity', $maximum_qty );
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_step_quantity', $step_qty );
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_value_limit_override', $override_val );
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_minimum_value', $minimum_val );
						YITH_WMMQ()->update_term_meta( $tag_id, '_ywmmq_tag_maximum_value', $maximum_val );
			...
			if ( 'delete' === $table->current_action() ) {
				$message = sprintf( _n( '%s tag removed successfully', '%s tags removed successfully', count( $_GET['id'] ), 'yith-woocommerce-minimum-maximum-quantity' ), count( $_GET['id'] ) );
			}
			...
			if ( isset( $_GET['id'] ) && ! empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
				...
				$item          = array(
					'ID'           => $_GET['id'],
					'excluded'     => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_exclusion', true ),
					'override_qty' => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_quantity_limit_override', true ),
					'minimum_qty'  => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_minimum_quantity', true ),
					'maximum_qty'  => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_maximum_quantity', true ),
					'step_qty'     => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_step_quantity', true ),
					'override_val' => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_value_limit_override', true ),
					'minimum_val'  => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_minimum_value', true ),
					'maximum_val'  => YITH_WMMQ()->get_term_meta( $_GET['id'], '_ywmmq_tag_maximum_value', true )
				);
				$value         = $_GET['id'];
		...
		public function get_table_id_wc_prefix() {
			...
			return ( YITH_WMMQ()->is_wc_lower_2_6 ) ? 'woocommerce_' : '';
