...
class YITH_YWRAC_Emails_List_Table extends WP_List_Table {
    ...
    function column_default( $item, $column_name ) {
        switch( $column_name ) {
            ...
            case 'message':
                ...
                break;
            case 'conversion':
                $email_sent      = apply_filters( 'ywrac_email_template_sent_counter', get_post_meta( $item->ID, '_email_sent_counter', true ), $item->ID );
                $recovered_carts = apply_filters( 'ywrac_email_template_cart_recovered', get_post_meta( $item->ID, '_cart_recovered', true ), $item->ID );
                if ( $email_sent != 0 ) {
    ...
    function column_cb($item) {
        return sprintf(
            '<input type="checkbox" name="ywrac_email_ids[]" value="%s" />',  $item->ID
        );
    }
