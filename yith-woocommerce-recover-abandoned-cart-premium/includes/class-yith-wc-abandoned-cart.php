...
if ( ! class_exists( 'YITH_WC_Recover_Abandoned_Cart' ) ) {
	...
	class YITH_WC_Recover_Abandoned_Cart {
		...
		public function __construct() {
			...
			$this->delete_abandoned_time = get_option( 'ywrac_delete_cart' ) * HOUR_IN_SECONDS;
			...
			add_action( 'woocommerce_order_status_completed', array( $this, 'remove_cart_after_order_create' ), 10 );
		...
		public function enqueue_styles_scripts() {
			if ( is_checkout() || apply_filters( 'yith_ywrac_include_frontend_scripts', false ) ) {
			...
			if ( ! empty( $cart ) ) {
				...
				if ( ! empty( $cart ) ) {
					$this->update_abandoned_cart( $cart->ID, $cart->post_date, array(
						...
						'user_phone'    => intval( $_POST['phone'] ),
						'user_currency' => isset( $_POST['currency'] ) ? $_POST['currency'] : $this->get_user_currency()
					) );
		...
		public function update_carts() {
			$start_to_date = (int) ( current_time( 'timestamp', 0 ) - $this->cutoff );
			$args          = array(
				'post_type'      => $this->post_type_name,
				'post_status'    => 'publish',
				'posts_per_page' => - 1,
		...
		public function get_cart_content( $cart_id, $cart_content, $lang, $type = 'cart', $currency ) {
			...
			if ( $type == 'cart' ) {
				...
				wc_get_template( 'cart_content.php', array( 'cart_content' => $cart_content, 'subtotal' => $subtotal, 'lang' => $lang, 'currency' => $currency ) );
			} elseif ( $type == 'order' ) {
				$order = wc_get_order( $cart_id );
				wc_get_template( 'pending-order-content.php', array( 'order' => $order, 'lang' => $lang, 'currency' => $currency ) );
			}
