	...
	<head>
		...
		<title>
			<?php
				 ...
				 */
				echo apply_filters( 'wc_pip_document_title', sprintf( esc_html( '%1$s - %2$s %3$s' ), SV_WC_Helper::get_site_name(), $document->name, $document->get_invoice_number() ), $type, $document, $order );
			?>
		</title>
		<?php
			/**
