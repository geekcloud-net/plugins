 ...
 *
 *
 * @class      YITH_ARS_New_Request_User_Email
 * @package    Yithemes
 * @since      Version 1.0.0
...
if ( ! class_exists( 'YITH_ARS_New_Request_User_Email' ) ) {
    /**
     * Class YITH_ARS_New_Request_User_Email
     *
     * @author Carlos Mora <carlos.eugenio@yourinspiration.it>
     */
    class YITH_ARS_New_Request_User_Email extends WC_Email {
        ...
        public function __construct() {
            ...
            $this->id             = 'yith_ywcars_new_request_user_email';
            $this->customer_email = true;
            $this->title          = _x( 'YITH Advanced Refund System: New request - email for user', 'Email descriptive title',
                'yith-advanced-refund-system-for-woocommerce' );
            $this->description    = __( 'The user will receive an email when a new request is submitted.', 'yith-advanced-refund-system-for-woocommerce' );
            $this->heading        = __( 'Request succesfully submitted', 'yith-advanced-refund-system-for-woocommerce' );
            $this->subject        = __( 'Request succesfully submitted', 'yith-advanced-refund-system-for-woocommerce' );
            $this->email_body     = __( 'Hi {customer_name}! Your request {request_number} for order {order_number} has been successfully submitted. You will receive a reply soon.', 'yith-advanced-refund-system-for-woocommerce' );
            $this->template_html  = 'emails/ywcars-email-for-user.php';

            add_action( 'ywcars_send_new_request_user', array( $this, 'trigger' ) );
        ...
        public function trigger( $request_id ) {
            ...
            $this->email_body = $this->get_option( 'email_body', __( 'Hi {customer_name}! Your request {request_number} for order {order_number}
            has been successfully submitted. You will receive a reply soon.', 'yith-advanced-refund-system-for-woocommerce' ) );
...
return new YITH_ARS_New_Request_User_Email();
