...
if( ! function_exists( 'yith_wcmap_get_endpoints' ) ) {
    ...
    function yith_wcmap_get_endpoints() {
	    ...
	    // compatibility fix with latest WC Membership version
	    if( class_exists( 'WC_Memberships' ) && isset( $return['view-membership'] ) ) {
		    $return['members-area'] = $return['view-membership'];
		    unset( $return['view-membership'] );
	    }
...
if( ! function_exists( 'yith_wcmap_get_plugins_endpoints' ) ) {
	...
	function yith_wcmap_get_plugins_endpoints( $key = '' ) {
		...
		if( file_exists( YITH_WCMAP_DIR . 'plugin-options/plugins-endpoints.php' ) ) {
			$endpoints = include( YITH_WCMAP_DIR . 'plugin-options/plugins-endpoints.php' );
		}
