...
<?php if( ! empty( $mailchimp_data['choices'] ) ): ?>
	<?php foreach( $mailchimp_data['choices'] as $id => $name ): ?>
		<input type="radio" value="<?php echo esc_attr( $name ) ?>" id="<?php echo esc_attr( $mailchimp_data['tag'] . '_' . $id ) ?>_<?php echo esc_attr( $id ) ?>" name="<?php echo esc_attr( $mailchimp_data['tag'] ) ?>"<?php echo ( $mailchimp_data['req'] ) ? 'required="required"' : '' ?>/>
