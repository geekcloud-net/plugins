...
class WPLE_Updater_Update_API_Check {
	...
	public function plugin_information( $args ) {
		...
		$transient_key = $args['request'] == 'pluginupdatecheck' ? 'wple_update_check_cache' : 'wple_update_info_cache';
		...
		// $api_url = add_query_arg( 'wc-api', 'upgrade-api', WPLEUP()->upgrade_url );
		...
		if ( is_object( $response ) ) {
			set_transient( $transient_key, $response, 1800 ); // cache response for 30 minutes
			return $response;
		} else {
			return false;
		}
