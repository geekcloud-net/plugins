...
class ProfilesTable extends WP_List_Table {
    ...
    function column_profile_name($item){
        ...
        $actions = array(
            ...
            'delete'    => sprintf('<a href="?page=%s&action=%s&profile=%s&_wpnonce=%s">%s</a>',$_REQUEST['page'],'delete_profile',$item['profile_id'], wp_create_nonce( 'delete_profile' ),__('Delete','wplister')),
        );
