	...
	<table>
		...
		<tr class="ywpi-order-number">
			<td class="left-content">
				<?php echo apply_filters('ywpi_invoice_number_label_for_credit_note',__( "Invoice number", 'yith-woocommerce-pdf-invoice' ),$document); ?>
			</td>
		...
		<?php if ( apply_filters( 'ywpi_template_invoice_data_table_order_amount_visible', true ) ) : ?>
			<tr class="invoice-amount">
				...
				<td class="right-content">
					<?php echo $invoice_details->get_order_currency( $refund_order, $refund_order->get_total() ); ?>
				</td>
			</tr>
