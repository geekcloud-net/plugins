...
$style = $hover_style = $btn_style_inline = $link_sufix = $link_prefix = $shadow_click = $shadow_color = $box_shadow = $main_extra_class = '';
...
if($btn_link !== ''){
    ...
    if($href['url'] !== ""){
        ...
        $link_prefix .= '<a class="porto-btn-link '.esc_attr($is_vc_49_plus).' '.esc_attr($btn_align).' '.esc_attr($btn_size).' '.esc_attr($main_extra_class).'" '. $link_title .' ' . $rel . ' href = "' . esc_url( $url ) .'" '.$target.'>';
        $link_sufix .= '</a>';
    }
}
if ($btn_font_family != '') {
...
$output .= '<button type="button" class="porto-btn '.esc_attr( $is_vc_49_plus ).' '.esc_attr($btn_size).' '.esc_attr($btn_hover).' '.esc_attr($el_class).'" data-hover="'.esc_attr($btn_title_color_hover).'" data-border-color="'.esc_attr($btn_color_border).'" data-bg="'.esc_attr($btn_bg_color).'" data-hover-bg="'.esc_attr($btn_bg_color_hover).'" data-border-hover="'.esc_attr($btn_color_border_hover).'" data-shadow-click="'.esc_attr($shadow_click).'" data-shadow="'.esc_attr($box_shadow).'" style="'.esc_attr($style).'">';

$output .= '<span class="porto-btn-hover" style="background-color:'.esc_attr($btn_bg_color_hover).'"></span>';
