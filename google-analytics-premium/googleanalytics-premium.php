...
final class MonsterInsights {
	...
	public function define_globals() {
		...
		if ( ! defined( 'GA_YOAST_PREMIUM_VERSION' ) ) {
			define( 'GA_YOAST_PREMIUM_VERSION', MONSTERINSIGHTS_VERSION );
		}
...
function monsterinsights_activation_hook( $network_wide ) {
	...
	if ( class_exists( 'MonsterInsights_Lite' ) ) {
		deactivate_plugins( plugin_basename( __FILE__ ) );
		wp_die( sprintf( esc_html__( 'Please uninstall and remove MonsterInsights Lite before activating MonsterInsights Pro. The Pro version has not been activated. %1$sClick here to return to the Dashboard%2$s.', 'ga-premium' ), '<a href="' . $url . '">', '</a>' ) );
	}
}
 ...
 * @return 	void
 */
function monsterinsights_uninstall_hook( $network_wide ) {
	wp_cache_flush();
