...
class WPLA_Update_API {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'wpla' ), '1.2' );
	}
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			...
			$this->ame_menu_tab_activation_title 	= __( 'License Activation', 'wpla' );
			$this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'wpla' );
	...
	public function check_for_new_version( $quiet = false ) {
		...
		$this->license_key   = get_option( 'wpla_api_key' );
		$this->license_email = get_option( 'wpla_activation_email' );
	...
	public static function license_inactive_notice() { 
		...
		<div id="message" class="error">
			<p><?php printf( __( 'The plugin license key has not been activated, so you won\'t be able to get updates and support! %sClick here%s to activate the license key and the plugin.', 'wpla' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wpla-settings&tab=license' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'wpla' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
